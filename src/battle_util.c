...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_WHITEOUT: // Boosts damage of Ice-type moves in hail
        if (moveType == TYPE_ICE && B_WEATHER_NONE && gBattleWeather & B_WEATHER_HAIL)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
	...
	if ((GetBattlerAbility(battlerDef) == ABILITY_HEATPROOF) && moveType == TYPE_WATER)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
            gBattleScripting.abilityPopupOverwrite = gLastUsedAbility = ABILITY_HEATPROOF;
            gBattleStruct->moveResultFlags[gBattlerTarget] |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_AVOIDED_DMG;
...
bool32 IsMovePowderBlocked(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (IsPowderMove(move) && (battlerAtk != battlerDef))
    {
        if (B_POWDER_GRASS >= GEN_6
         && (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS) || GetBattlerAbility(battlerDef) == ABILITY_OVERCOAT || GetBattlerAbility(battlerDef) == ABILITY_GROOMEDCOAT))
        {
            gBattlerAbility = battlerDef;
            RecordAbilityBattle(gBattlerTarget, ABILITY_OVERCOAT || ABILITY_GROOMEDCOAT);
            effect = TRUE;
        }

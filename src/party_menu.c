...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    // FLY - Special case: can be used without a badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_FLY) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_FLY) == ALREADY_KNOWS_MOVE))
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    }

    // SURF - Check if the Pokémon can learn Surf and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_SURF) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_SURF) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE05_GET)) // Badge for Surf
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 4 + MENU_FIELD_MOVES);
    }

    // FLASH - Check if the Pokémon can learn Flash and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_FLASH) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_FLASH) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE01_GET)) // Replace with the appropriate badge flag for Flash
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);
    }

    // DIVE - Check if the Pokémon can learn Dive and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_DIVE) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_DIVE) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE07_GET)) // Replace with the appropriate badge flag for Dive
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 6 + MENU_FIELD_MOVES);
    }

    // WATERFALL - Check if the Pokémon can learn Waterfall and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_WATERFALL) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_WATERFALL) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE08_GET)) // Replace with the appropriate badge flag for Waterfall
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 7 + MENU_FIELD_MOVES);
    }

    // ROCK SMASH - Check if the Pokémon can learn Rock Smash and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_ROCK_SMASH) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_ROCK_SMASH) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE02_GET)) // Replace with the appropriate badge flag for Rock Smash
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 2 + MENU_FIELD_MOVES);
    }

    // STRENGTH - Check if the Pokémon can learn Strength and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_STRENGTH) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_STRENGTH) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE04_GET)) // Replace with the appropriate badge flag for Strength
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 3 + MENU_FIELD_MOVES);
    }

    // CUT - Check if the Pokémon can learn Cut and player has the badge
    if (sPartyMenuInternal->numActions < 5 &&
        (CanTeachMove(&mons[slotId], MOVE_CUT) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_CUT) == ALREADY_KNOWS_MOVE) &&
        FlagGet(FLAG_BADGE01_GET)) // Replace with the appropriate badge flag for Cut
    {
...
static void CursorCb_FieldMove(u8 taskId)
{
    ...
    else
    {
        ...
        if (fieldMove <= FIELD_MOVE_WATERFALL && fieldMove != FIELD_MOVE_FLY && FlagGet(FLAG_BADGE01_GET + fieldMove) != TRUE)
        {
...
static bool8 SetUpFieldMove_Surf(void)
{
    // Check if the selected Pokemon knows or can learn Surf
    struct Pokemon *mon = &gPlayerParty[gPartyMenu.slotId];
    bool8 canUseSurf = FALSE;

    // Check if the Pokemon knows Surf
    if (MonKnowsMove(mon, MOVE_SURF))
        canUseSurf = TRUE;
    // Check if the Pokemon can learn Surf (has it in the action list)
    else if (CanLearnTeachableMove(GetMonData(mon, MON_DATA_SPECIES), MOVE_SURF))
        canUseSurf = TRUE;

    if (canUseSurf && IsPlayerFacingSurfableFishableWater() == TRUE)
    {

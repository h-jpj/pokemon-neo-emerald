...
static void Cmd_perseveranceattackselect(void);
...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_perseveranceattackselect,                      //0xDE
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if (defAbility == ABILITY_PRIM_AND_PROPER && IS_MOVE_STATUS(move) && moveAcc > 50)
        moveAcc = 50;
    ...
    case ABILITY_INVISIBILITY:
        if (gBattleMons[battlerDef].hp <= (gBattleMons[battlerAtk].maxHP / 3))   
            calc = (calc * 50) / 100; // 1.5 tangled feet loss
        break;
    }
...
static void Cmd_setcharge(void)
{
    ...
    // Ensure gCurrentMove is MOVE_KINDLE for kindled logic
    if (gCurrentMove == MOVE_KINDLE)
    {
        // Apply the "kindled" status using a valid bit shift
        gStatuses3[battler] |= STATUS4_KINDLED_UP;  // Make sure STATUS4_KINDLED_UP uses a valid bit within 0-31.
    }
    else
    {
        // Apply the "charged up" status
        gStatuses3[battler] |= STATUS3_CHARGED_UP;
    }

    // Set the shared charge timer
    gDisableStructs[battler].chargeTimer = 2;

    // Move to the next script instruction
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
//perseveranceattackselect and assistattackselect logic has been combined
//both moves work as intended. I dont know how to extend the battlescript table
//i can't read asm for shit- still very new to this :/
static void Cmd_perseveranceattackselect(void)
{
    CMD_ARGS(const u8 *failInstr);
    
    u16 move = gCurrentMove; // The current move being used
    s32 chooseableMovesNo = 0;
    u16 *validMoves = NULL;

    // Allocate memory for valid moves based on move logic
    if (move == MOVE_ASSIST)
    {
        validMoves = Alloc(sizeof(u16) * PARTY_SIZE * MAX_MON_MOVES);
        if (validMoves == NULL)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
            return;
        ...
        // Assist logic
        struct Pokemon *party = GetBattlerParty(gBattlerAttacker);
        for (s32 monId = 0; monId < PARTY_SIZE; monId++)
        {
            if (monId == gBattlerPartyIndexes[gBattlerAttacker])
                continue;
            if (GetMonData(&party[monId], MON_DATA_SPECIES_OR_EGG) == SPECIES_NONE)
            ...
            for (s32 moveId = 0; moveId < MAX_MON_MOVES; moveId++)
            {
                u16 move = GetMonData(&party[monId], MON_DATA_MOVE1 + moveId);
                if (gMovesInfo[move].assistBanned)
                    continue;
    ...
    else if (move == MOVE_PERSEVERANCE)
    {
        validMoves = Alloc(sizeof(u16) * MAX_MON_MOVES);
        if (validMoves == NULL)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
            return;
        ...
        for (s32 moveId = 0; moveId < MAX_MON_MOVES; moveId++)
        {
            u16 currentMove = gBattleMons[gBattlerAttacker].moves[moveId];
            if (currentMove == MOVE_NONE || currentMove == MOVE_PERSEVERANCE) // Skip Perseverance
                continue;

            validMoves[chooseableMovesNo++] = currentMove;
        }
    }

    // Check if any valid moves were found
    if (chooseableMovesNo > 0)
    {
        ...
        if (move == MOVE_PERSEVERANCE)
        {
            // Set HP to 1 for Perseverance
            gBattleMons[gBattlerAttacker].hp = 1;

            // Force HP bar to visually drop to 1 HP
            gBattleMoveDamage = gBattleMons[gBattlerAttacker].maxHP - 1;
    ...
    {
        // No valid moves found, handle failure
        gBattlescriptCurrInstr = cmd->failInstr;
    }
